...
db.collection("Usuarios")
  ...
  .get()
  .then((responseConfig) => {
    logger("MercadoLibre Traspasos", "Ingreso");
    ...
    const tienePermiso = response.modulos.find(
      (modulo) => modulo.modulo == "MercadoLibre Traspasos"
    );
...
db.collection("ConfiguracionesGenerales")
  ...
  .onSnapshot((response) => {
    ...
    datos.referenciasPublicaciones.forEach((response_data) => {
      ...
      publicacionML += `<option value="${response_data.replaceAll('"','')}" >${response_data.replaceAll('"','')}</option>`;

    });

    // Almacenamos los options para replicarlos cada que se cree un nuevo select de Publicaiones de MercadoLibre
    optionsPublicacionesML = publicacionML;
    console.log('cargado');
...
const listarRegistro = (response, cantidad = 10, permisos) => {
  ...
  var contenido = `
    ...
    <thead>
        <tr>
            <th class="align-middle">Número Órden de Venta</th>
            <th class="align-middle">Cliente</th>
            <th class="align-middle">Estatus de Pedido</th>
            <th class="align-middle">Lista Productos</th>
            <th class="align-middle">Publicacion MercadoLibre</th>
  ...
  response.forEach((response_data) => {
    ...
    let listaProductosML = "";

    elementos?.forEach((item) => {
      if (item) {
        listaProductosAlmacen += `<li> ${item.categoria} -> ${item.producto} -> ${item.variacion}: <b>${item.cantidad} </b></li>`;
        listaProductosML += `<li> ${item.tituloPublicacion} -> ${item.idPublicacion} -> ${item.idVariacion} -> ${item.idFull}-> <b>${item.skuPublicacion} </b></li>`;
      }
    ...
    contenido += `
                <tr id="${response_data.id}">
                    ...
                    <td>${cliente}</td>
                    <td>${EstatusPedido}</td>
                    <td><ul>${listaProductosAlmacen}</ul></td>
                    <td><ul>${listaProductosML}</ul></td>
                    <td>${comentarios ?? ""}</td>
                    <td>
                      <button class="btn btn-dark" onclick="generarEtiquetas('#barcode', '${response_data.id}')" title="Generar QR"><i class="fas fa-qrcode"></i></button>
                      <button class="btn btn-warning" onclick="${EstatusPedido != "Pedido cerrado"
        ? `selectCatalogos()`
        : ""
      }
      ...
      } " data-toggle="modal" data-target="#${EstatusPedido != "Pedido cerrado" ? "modal-editar" : "modal-editarCerrado"
        ...
        : ""
                      ...
                      <button class="btn btn-info" onclick="detalleVenta('${numero_orden_venta}')" title="Detalle de venta" data-toggle="modal" data-target="#modal-detalle"><i class="fas fa-eye"></i></button>
                      ${EstatusPedido == "Pedido cerrado"
        ? `
                          ${puedeRegresarVenta
        ...
        : ""
                      ...
                      <button class="btn btn-primary" onclick="cargarImagen('${response_data.id
...
/**************************************************/
const detalleVenta = (numero_orden_venta) => {
  db.collection("Ventas")
    ...
    .then((response) => {
      ...
      let divtblDetalle = document.getElementById("divtblDetalle");
  ...
  db.collection("Ventas")
    ...
    .then((response) => {
      ...
      const llenaDatosEditar = () => {
        ...
        elementos.forEach((item) => {
          const { publicacionML, cantidad, categoria, producto, variacion } =
            item;
          const campo = `
          <tr id="td_edit${nextinputEdit}">
              ...
              <td><select class="form-control publicacionMLSelect select2" id="publicacionMlList_editar_${nextinputEdit}">${optionsPublicacionesML}</select></td>
          ...
          document.getElementById(
            `publicacionMlList_editar_${nextinputEdit}`
          ).value = publicacionML;

          $("#publicacionMlList_editar_" + nextinputEdit).select2({
...
const selectPublicacionesML = (index, op = '') => {

  switch (op) {
    case '':
      $("#publicacionMlList_" + index).html(optionsPublicacionesML);
      $("#publicacionMlList_" + index).select2({
        placeholder: 'Seleccionar',
        width: "450px"
      });
      break;
    ...
    case 'editar':
      $("#publicacionMlList_editar_" + index).html(optionsPublicacionesML);
      $("#publicacionMlList_editar_" + index).select2({
        placeholder: 'Seleccionar',
        width: "450px"
      });
      break;
...
const selectVariacion = (value, index, isEdit) => {
  console.log({ value, index, isEdit })
...
function agregarCampos() {
  ...
  campo = `
    <tr id="td_${nextinput}">
        ...
        <td><select class="form-control publicacionMLSelect select2" id="publicacionMlList_${nextinput}" data-id="${nextinput}"></select></td>
  ...
  // Consultamos nuevamente las Publicaciones de MercadoLibre
  selectPublicacionesML(nextinput);
      ...
      if (total_registros > 0) {
        ...
        for (let i = 1; i <= nextinput; i++) {
          ...
          if (tdExist) {
            ...
            var publicacionML = $("#publicacionMlList_" + i).val();
            var variaciones = publicacionML.split("|");
            var tituloPublicacion = variaciones[0];
            var skuPublicacion = variaciones[1];
            var idPublicacion = variaciones[2];
            var idVariacion = variaciones[3] ? variaciones[3] : "";
            var idFull = variaciones[4];
            var precioUnitario = 0;

            //console.log({publicacionML, variaciones, tituloPublicacion, skuPublicacion, idPublicacion, idVariacion, idFull})

      
            if(!idFull){
              createSw({
                title: "Error",
                text: `El producto ${tituloPublicacion} no tiene idFull`,
                icon: "error",
              });
              isValidData = false;
              break;
            ...
            //console.log({categoria, producto, variacion, cantidad, publicacionML, idPublicacion, tituloPublicacion, skuPublicacion, idVariacion, idFull})
...
const agregarCamposEditar = () => {
  campo = `
    <tr id="td_edit${nextinputEdit}">
        ...
        <td><select class="form-control publicacionMLSelect select2" id="publicacionMlList_editar_${nextinputEdit}" data-id="${nextinputEdit}"></select></td>
        <td><button type='button' class="btn btn-danger" onclick="borrarProductoAgregado_editar(${nextinputEdit})"><i class="far fa-trash-alt"></i></button></td>
    </tr>
    `;
  $("#catalogosTable_editar>tbody").append(campo);
  ...
  // Consultamos nuevamente las Publicaciones de MercadoLibre
  selectPublicacionesML(nextinputEdit, 'editar');
...
const guardarOrdenEditada = () => {
  if (isValidForm(true)) {
    ...
    for (var i = 1; i < nextinputEdit; i++) {
      ...
      var publicacionML = $("#publicacionMlList_editar_" + i).val();
      var variaciones = publicacionML.split("|");
      var tituloPublicacion = variaciones[0];
      var skuPublicacion = variaciones[1];
      var idPublicacion = variaciones[2];
      var idVariacion = variaciones[3];
      var idFull = variaciones[4];
