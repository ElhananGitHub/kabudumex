/*
  ...
  */
  ...
  const TABLE_NAME = "Mercadolibre Inventario";
  const TABLE_NAME2 = "MercadoLibreInventario";
  let headersSchema = {};
  
  var baseDeDatos = TABLE_NAME2;
  ...
  db.collection("Usuarios")
    ...
    .then((responseConfig) => {
      ...
      if (tienePermiso) {
        ...
        db.collection(TABLE_NAME).onSnapshot((response) => {
  ...
  function submitInfo() {
    ...
    if (enteredInfo?.ID) {
      db.collection(TABLE_NAME2).doc(enteredInfo.ID).update(enteredInfo);
    } else {
      db.collection(TABLE_NAME2).add(enteredInfo);
    }
    //Se revisa si esta configurado el envio por mail del documento
    var dbConfig = datosFormularios["Configuraciones Enviar Mail"];
    var config = dbConfig.find((e) => e.indexOf(baseDeDatos) > -1);
  ...
  function eliminarRegistro(tableIndex, id) {
    ...
    var text = "Seguro que desea eliminar el registro " + id;
    if (confirm(text) == true) {
      alertaSnackbar("Enviando Peticion", 1000);
  
      //Borrar registro
      db.collection(TABLE_NAME2).doc(dataJson.ID).delete();
  ...
  reader.onload = (e) => {
    ...
    Promise.all(
      result.map((response_data, index) => {
        ...
        if ("fecha_publicacion" in datosExcel) {
          fecha_publicacion = datosExcel.fecha_publicacion;
        }
        if ("cantidad_comprada" in datosExcel) {
          cantidad_comprada = datosExcel.cantidad_comprada;
        }
        if ("costo_unitario" in datosExcel) {
          costo_unitario = datosExcel.costo_unitario;
        }
        if ("ventas" in datosExcel) {
          ventas = datosExcel.ventas;
        }
        if ("cancelaciones" in datosExcel) {
          cancelaciones = datosExcel.cancelaciones;
        }
        if ("inventario" in datosExcel) {
          inventario = datosExcel.inventario;
        }
        if ("total_ventas" in datosExcel) {
          total_ventas = datosExcel.total_ventas;
        }
        if ("total_comisiones" in datosExcel) {
          total_comisiones = datosExcel.total_comisiones;
        }
        ...
        if (sku) {
          ...
          return db
          .collection("MercadoLibreInventario")
          .add(skuAgregar)
          .then(() => {
            let idIndexCantidad =
      ...
      logger("Inventario ML", "Subir mercancia (Excel)", null, {
